#!/usr/bin/env bash
#
# main source code provided by Sinésio Neto, please see:
# https://github.com/sineto/nmcli-rofi
#
# the provided implementation differs a bit from the 
# original one: some changes have been introduced to the 
# connection process: rofi now shows if the connection
# existed before, and removes the connection in case the
# user tries to enter a new password. and the theme is 
# applied everywhere
#

ROFI_STYLES_DIR="$HOME/.config/rofi/wifi/styles"
ROFI_THEME='main'
...
function rofi_cmd () {
  ...
  echo -e "$1" | uniq -u | rofi -dmenu -p "Wi-Fi SSID" -lines "$LINENUM" \
    ...
    -theme ${ROFI_STYLES_DIR}/${ROFI_THEME}.rasi
}
...
function main () {
    ...
    if [[ "$OPS" =~ 'Disable' ]]; then
      ...
      MSSID=$(echo -en "" | rofi -dmenu -p "SSID" -mesg "Enter the SSID of the network" \
        -lines 0 -theme ${ROFI_STYLES_DIR}/${ROFI_THEME}.rasi)
	...
	nmcli con delete "$MSSID"
	  ...
	  EXISTING_CONN=''
	  if [[ ! -z $(nmcli con | grep $CHSSID) ]]; then
	    EXISTING_CONN='(existing connection found)'
	  fi

          WIFIPASS=$(echo -en "" | rofi -dmenu -password -p "PASSWORD" \
            -mesg "Enter the PASSWORD of the network $EXISTING_CONN" -lines 0 \
	    -theme ${ROFI_STYLES_DIR}/${ROFI_THEME}.rasi )
        fi
